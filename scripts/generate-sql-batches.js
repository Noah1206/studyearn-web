...
function generateSqlBatch(batch, batchNum) {
...
INSERT INTO schools (name, type, region, district, address, latitude, longitude, neis_code, edu_office_code, high_school_type, foundation_type, coedu_type, location)
VALUES
  ${values}
ON CONFLICT (neis_code) DO NOTHING;`;
}
...
for (let i = 0; i < newSchools.length; i += BATCH_SIZE) {
  ...
  batches.push({
    num: batchNum,
    sql: generateSqlBatch(batch, batchNum),
    count: batch.length
  });
}
...
const outputDir = path.join(__dirname, 'sql-batches');
...
batches.forEach(batch => {
  fs.writeFileSync(
    path.join(outputDir, `batch-${String(batch.num).padStart(3, '0')}.sql`),
    batch.sql
  );
});
...
// Output first batch for testing
console.log('\n--- First batch SQL (for testing) ---\n');
console.log(batches[0].sql.substring(0, 2000) + '...');
